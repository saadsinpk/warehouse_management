...
//        echo '<pre>';        print_r($sales_report);die();
        ...
        if (!empty($sales_report)) {
            ...
            foreach ($sales_report as $k => $v) {
                $i++;
                $sales_report[$k]['sl'] = $i;
                $sales_report[$k]['sales_date'] = $CI->occational->dateConvertMyformat($sales_report[$k]['date']);
                $sales_amount = $sales_amount + $sales_report[$k]['total_amount'];
                if($sales_report[$k]['report_invoice_status']==0){
                    $sales_report[$k]['report_invoice_status']='<button  class="btn btn-warning">Processing</button>';
                }else{
                    $sales_report[$k]['report_invoice_status']='<button class="btn btn-success">Dispatch</button>';
                }
            }
        }
        ...
        if (!empty($purchase_report)) {
            ...
            foreach ($purchase_report as $k => $v) {
                ...
                $purchase_report[$k]['prchse_date'] = $CI->occational->dateConvertMyformat($purchase_report[$k]['purchase_date']);
    ...
    public function total_profit_report($links, $per_page, $page) {
        ...
        if (!empty($total_profit_report)) {
            ...
            foreach ($total_profit_report as $k => $v) {
                ...
                $total_profit_report[$k]['prchse_date'] = $CI->occational->dateConvertMyformat($total_profit_report[$k]['date']);
    ...
    public function get_products_report_sales_view($links, $per_page, $page) {
        ...
        if (!empty($product_report)) {
            foreach ($product_report as $k => $v) {
                $product_report[$k]['sales_date'] = $CI->occational->dateConvertMyformat($product_report[$k]['date']);
                $sub_total = $sub_total + $product_report[$k]['total_amount'];
                if($product_report[$k]['invoice_status_p_wise']==0){
                    $product_report[$k]['invoice_status_p_wise']='<button  class="btn btn-warning">Processing</button>';
                }else{
                    $product_report[$k]['invoice_status_p_wise']='<button class="btn btn-success">Dispatch</button>';
                }
            }
        }
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
