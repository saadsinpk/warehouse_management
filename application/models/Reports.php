...
//    ========= its for last month or 30 days  sales and purchase amount ================
    ...
    public function todays_sales_report() {
        $today = date('Y-m-d');
        $this->db->select("a.*,a.status as report_invoice_status,b.customer_id,b.customer_name,loc.location_name");
        $this->db->from('invoice a');
...
//echo $dateRange;die();
    ...
    public function retrieve_product_sales_report($perpage, $page) {
        $this->db->select("a.*,c.status as invoice_status_p_wise,b.product_name,b.product_model,c.date,c.total_amount,d.customer_name,loc.location_name");
        $this->db->from('invoice_details a');
        $this->db->join('product_information b', 'b.product_id = a.product_id');
        $this->db->join('invoice c', 'c.invoice_id = a.invoice_id');
        $this->db->join('locations_tbl loc', 'loc.location_id = c.store_id');
